...
public abstract class CoverageResultFormatter extends ResultFormatter {
				...
				if (!columns.containsKey(key)) {
					try {
						...
						try {
							...
							if (!columnsval.get(key)
									.containsKey(feature.getJSONObject("properties").get(key).toString())) {
								columnsval.get(key).put(feature.getJSONObject("properties").get(key).toString(), 0);
							}
							columnsval.get(key).put(key, columnsval.get(key).put(
									feature.getJSONObject("properties").get(key).toString(),
									columnsval.get(key).get(feature.getJSONObject("properties").get(key).toString())
											+ 1));
	...
	public static JSONObject addCovJSONParameterAndRange(JSONObject covjson, String parameterName, String parameterURI,
			List<Object> values, String dataType) {
